...
Inside the <article>, we have an <img> element that displays the movie poster image. 
The src attribute is set to the poster property of the movie object, and the alt attribute is set to the movie title followed by "poster" for accessibility.
...
Inside the generateMoviesHTML function, we use a for...of loop to go through each movie object and 
generate the corresponding HTML structure using template literals.
We dynamically insert the movie poster, title, and description into the appropriate elements.
The generated HTML markup for each movie is concatenated into the movieHTML string, 
which is then returned by the function. This allows us to obtain the complete HTML markup for all the movies.
